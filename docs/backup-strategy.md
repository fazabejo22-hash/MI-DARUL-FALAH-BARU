...
Dokumen ini mendefinisikan prosedur dan kebijakan backup operasional untuk Sistem Informasi Terpadu Madrasah Ibtidaiyah Darul Falah.

## 1. Lingkup Data yang Di-backup
- **Database (PostgreSQL / MySQL)**: Seluruh data relasional termasuk akun pengguna, data akademik (siswa, guru, kelas, absensi, nilai, rapor), serta modul CMS & PPDB.
...
- **Backup Database Harian (Daily Backup)**: Dijalankan setiap pukul 01:00 WIB secara otomatis via skrip cron job atau backup manager cloud. Retensi penyimpanan: 30 hari.
...
## 3. Skrip Backup Otomatis Database (Contoh)
```bash
#!/bin/bash
...
PGPASSWORD="$DB_PASS" pg_dump -h $DB_HOST -U $DB_USER $DB_NAME | gzip > "$BACKUP_DIR/db_$TIMESTAMP.sql.gz"
